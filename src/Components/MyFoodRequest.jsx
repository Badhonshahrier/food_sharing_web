...
const { data: foodRequest = [] } = useQuery({
  ...
  return (
    <div className="max-w-7xl mx-auto p-5">
        ...
        {foodRequest?.map((request) => (
          ...
          >
            <div className="p-5 space-y-3">
              ...
              <p>
                <span className="font-bold flex items-center gap-2">
                  <BsCalendarDateFill color="blue" size={20} /> Expire Date :{" "}
                  {new Date(request.expiryDate).toLocaleString()}
                </span>
              </p>
              <p>
                <span className="font-bold flex items-center gap-2">
                  <RiGitPullRequestFill color="teal" size={20} /> Request Date :{" "}
      ...
      {foodRequest.length === 0 && (
        <div className="text-center bg-base-300 w-3/6 h-40 flex flex-col justify-center rounded-4xl mx-auto mt-20 text-gray-500">
